...
def rollout_episode(env, policy, seed, action_chunk, device):
    ...
    while not done:
        if chunk_buf is None or step_in_chunk >= EXECUTE_STEPS: # Job 1
...
def rollout_and_relabel(policy, difficulty, num_episodes, pipe_speed, seed,
    ...
    for ep in range(num_episodes):
        states, actions = rollout_episode(env, policy, seed + ep,
                                          action_chunk, device)
        # window into (state, action_chunk) training pairs
        det_expert.reset()
        expert_actions = [det_expert.act(s) for s in states]

        for i in range(len(states) - action_chunk + 1):
            new_states.append(states[i])
            new_actions.append(expert_actions[i:i + action_chunk])
