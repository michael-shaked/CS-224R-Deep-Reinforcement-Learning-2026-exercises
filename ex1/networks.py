...
class FlowMatchingSchedule:
    ...
    def interpolate(self, x1, t):
        ...
        # ============================================================
        rnd = torch.randn(x1.shape).to(self.device)
        t = t.view(-1, 1)
        xt = t * x1 + rnd * (1 - t)
        return xt, (x1 - rnd)
    ...
    def sample(self, model, state):
        ...
        for i in range(self.num_steps):
            tau = torch.full((B,), i * dt, device=self.device)
            v = model(x, state, tau)
            x = x + dt * v
...
class BCPolicy(nn.Module):
    ...
    def __init__(self, state_dim: int = 4, action_dim: int = 20, hidden: int = 256):
        ...
        self.ln1 = nn.Linear(state_dim, hidden)
        self.relu1 = nn.ReLU()
        self.ln2 = nn.Linear(hidden, hidden)
        self.relu2 = nn.ReLU()
        self.ln3 = nn.Linear(hidden, action_dim)
    ...
    def forward(self, state):
        ...
        ln1_res = self.relu1(self.ln1(state))
        ln2_res = self.relu2(self.ln2(ln1_res))
        return torch.sigmoid(self.ln3(ln2_res))
